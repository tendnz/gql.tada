...
> = Query extends IntrospectionQuery
  ...
  : Query extends SchemaLike
    ? Omit<Query, 'types'> & { types: mapEnums<Query['types'], Enums> }
    : Query;

type mapEnums<Types extends SchemaLike['types'], Enums extends EnumsLike = DefaultEnums> = {
  [P in keyof Types]: Types[P] extends infer Type
    ? Type extends { kind: 'ENUM'; name: string }
      ? Type['name'] extends keyof Enums
        ? Omit<Type, 'type'> & { type: Enums[Types[P]['name']] }
        : Types[P]
      : Types[P]
    : never;
};
